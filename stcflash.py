...
class Programmer:
    ...
    def __model_database(self, model):
        modelmap = {
            ...
            0xF52C: (
                ...
                8,
                {
                    (0x2C, 0x2E): ("1K", "PWM"),  # STC15W1KPWM系列
                },
            ),
            0xF52E: (
                "15W",
            ...
            0xF56C: (
                ...
                58,
                {
                    (0x6C, 0x6D): ("4K", "S4-Student"),  # STC15W4KS4系列
                },
            ),
            0xF57E: (
                "15U",
            ...
            0xF750: (
                ...
                2,
                {
                    (0x50, 0x56): ("1K", "-20/16pin"),  # STC8G1K系列
                },
            ),
            0xF756: (
                "8G",
                17,
                {
                    (0x56, 0x57): ("1K", "-20/16pin"),  # STC8G1K系列
                },
            ),
            0xF760: (
                "8G",
    ...
    def detect(self):
        ...
        if self.protocol is None:
            try:
                self.protocol = {
                    0xF0: PROTOCOL_89,  # STC89/90C5xRC
        ...
        if self.protocol in PROTOSET_8:
            self.fosc = (
                dat[0] * 0x1000000 + dat[1] * 0x10000 + dat[2] * 0x100
            ) / 1000000
            ...
            self.fosc = (
                dat[7] * 0x1000000 + dat[8] * 0x10000 + dat[9] * 0x100
            ) / 1000000
            ...
            self.fosc = (
                float(sum(dat[0:16:2]) * 256 + sum(dat[1:16:2]))
                / 8
                * self.conn.baudrate
                / 580974
            )
        ...
        if (
            self.protocol in PROTOSET_PARITY
            or self.protocol in PROTOSET_8
            or self.protocol in PROTOSET_15
        ):
            self.chkmode = 2
            self.conn.parity = serial.PARITY_EVEN
        else:
        ...
        for i in range(0, len(self.info), 16):
            logging.info(
                "Info string [%d]: %s"
                % (i // 16, " ".join(["%02X" % j for j in self.info[i : i + 16]]))
            )
    ...
    def print_info(self):
        ...
        if self.protocol == PROTOCOL_89:
            ...
            switches = [
                (6, 0x40, "Disable reset2 low level detect"),
    ...
    def handshake(self):
        ...
        if self.protocol in PROTOSET_8:
            ...
            for baud in [
                ...
                logging.info(
                    "Test baudrate %d (accuracy %0.4f) using config %s"
                    % (
                        baud,
                        abs(round(t) - t) / t,
    ...
    def erase(self):
        if self.protocol in PROTOSET_89:
            ...
            self.send(
                0x84,
                (
                    [0x00, 0x00, self.romsize * 4, 0x00, 0x00, self.romsize * 4]
                    + [0x00] * 12
            ...
            cmd, dat = self.recv(10)
            if dat:
                logging.info("Serial number: " + " ".join(["%02X" % j for j in dat]))
    ...
    def options(self, **kwargs):
        ...
        if self.protocol == PROTOCOL_89:
            ...
            dat = self.info[6:9] + [0xFF] * 5 + self.info[10:11] + [0xFF] * 6 + fosc
    ...
    def unknown_packet_2(self):
        if (
            self.protocol not in PROTOSET_PARITY
            and self.protocol not in PROTOSET_8
            and self.protocol not in PROTOSET_15
        ):
            for i in range(5):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Stcflash, a command line programmer for "
            + "STC 8051 microcontroller.\n"
            + "https://github.com/laborer/stcflash"
        )
    )
    parser.add_argument(
        "image", help="code image (bin/hex)", type=argparse.FileType("rb"), nargs="?"
    )
    parser.add_argument(
        "-p", "--port", help="serial port device (default: %s)" % port, default=port
    )
    parser.add_argument(
        "-l",
        "--lowbaud",
    ...
    parser.add_argument("-m", "--aispmagic", help="magic word for AutoISP")
    parser.add_argument("-v", "--verbose", help="be verbose", default=0, action="count")
    parser.add_argument(
        "-e",
        "--erase_eeprom",
        help=("erase data eeprom during next download" + "(experimental)"),
        action="store_true",
    )
    parser.add_argument(
        "-ne",
        "--not_erase_eeprom",
        help=("do not erase data eeprom next download" + "(experimental)"),
        action="store_true",
    )

    opts = parser.parse_args()

    opts.loglevel = (logging.CRITICAL, logging.INFO, logging.DEBUG)[
        min(2, opts.verbose)
    ]
    ...
    with serial.Serial(
        port=opts.port, baudrate=opts.lowbaud, parity=serial.PARITY_NONE
    ) as conn:
        if opts.aispmagic:
